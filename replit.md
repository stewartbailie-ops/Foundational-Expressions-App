...
8. **Background patterns** — 7 CSS-generated patterns + 8 Premium image patterns (Lattice, Stone, Carbon Gold, Smoke, Hextech, Topographic, Pentagons, Neon Wave) with intensity slider (`patternOpacity` 5–100). Image patterns live in `client/public/patterns/` (registry: `registry.json`, mirrored as `IMAGE_PATTERN_OPTIONS` in `themeUtils.ts` — that constant is the runtime source of truth). Selecting an image stores `imagePatternKey` on the advisor/profile row; selection takes precedence over `backgroundStyle`. Picker UI gates the image grid behind `isPremiumActive(advisor)` from `usePremium()`. **Critical**: any new advisor field that needs to reach the public profile must be added to `PUBLIC_ADVISOR_FIELDS` in `server/routes.ts` — that allowlist silently strips anything not listed (same gotcha as the S5 fix).
